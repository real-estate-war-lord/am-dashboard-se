...
import { upstreamFor } from "./fixtures/sources.js";
...
function freshSnapshots(when = new Date()) {
  /* Every snapshot source is stubbed with the Arena fixture — the envelope
   * does not care which adapter produced a snapshot, only that it has
   * fetchedAt and listings. */
  const out = {};
...
test("the response lists every source: HomeQ plus every portal", async () => {
  ...
  assert.equal(r.body.sources.length, 1 + SNAPSHOT_SOURCES.length);
...
test("a portal source reports its snapshot's fetchedAt", async () => {
  /* Relative, not a literal. This was `new Date("2026-09-24T11:30:00.000Z")`,
     which was fresh the hour it was written and went stale a few hours later —
     the three-hour rule the next test exercises turned this one red on its own,
     with no code change. The neighbouring staleness tests already build their
     dates from Date.now(); this one now does too. */
  const when = new Date(Date.now() - 30 * 60 * 1000);   /* half an hour old: fresh */
...
test("a missing snapshot is ok:false, not an empty success", async () => {
  ...
  for (const src of SNAPSHOT_SOURCES.map((p) => p.src)) {
...
test("the radius still trims the portal listings", async () => {
  ...
  assert.equal(r.body.listings.length, 2 * SNAPSHOT_SOURCES.length);
...
test("/health names every source", async () => {
  ...
  assert.deepEqual(r.body.sources, ["homeq", ...SNAPSHOT_SOURCES.map((p) => p.src)]);
});
...
test("the scheduled handler writes every snapshot", async () => {
  ...
  const pending = [];
  const real = globalThis.fetch;
  globalThis.fetch = upstreamFor(PAYLOAD);
  try {
